...
for (segment of segments) {
    if (!inTemplate && !inChar && segment === '"' && lastSegment[lastSegment.length-1] !== '\\') {
        inString = !inString;
        if (!inString) {
    ...
    } else if (!inTemplate && !inString && segment === "'" && lastSegment[lastSegment.length-1] !== '\\') {
        inChar = !inChar;
        if (!inChar) {
        ...
        stringSegments.push(segment);
    } else if (!inString && !inChar && segment === "`" && lastSegment[lastSegment.length-1] !== '\\') {
